...
| NYISO P-7 ISO Load Forecast archive | NYISO MIS P-7 / `isolf` | Hudson Valley / `HUD VL` | NYISO load-forecast predictor | Daily multi-day forecast artifacts are preserved; the public P-7 interface associates each artifact with a timezone-specific `Last Updated` timestamp. The project treats this timestamp as inferred public availability evidence | Capture P-7 `Last Updated` metadata for 2020–2024, retain ZIP-entry timestamps as secondary audit evidence, and select latest eligible forecast strictly before 5:00 a.m. EPT D−1 |
| NYISO NY Load Forecast Custom Reports | NYISO | Hudson Valley / zonal forecasts | Secondary historical forecast route and cross-check | NYISO directed project here; exact field/revision semantics not fully established | Compare with P-7/`isolf` where helpful; do not replace P-7 timing evidence without stronger documentation |
...
| NYISO follow-up response, Case `00145063` | NYISO correspondence | Received 2026 | NYISO directed project to MT-201 NYMOC course materials and stated it cannot provide research-project consultation | Useful for documenting support boundary; contains private contact information | Ask only narrow technical source-definition questions if needed |
| NYISO P-7 public report index | NYISO MIS | Current public interface reviewed September 2026 | Each dated ISO Load Forecast artifact is paired with a timezone-specific `Last Updated` timestamp; current examples show updates commonly around 7–8 a.m. on the prior day | NYISO has not separately confirmed that `Last Updated` is formally defined as the public-publication timestamp; project treats it as a public-source inferred availability timestamp | Capture historical P-7 metadata across 2020–2024 and revise only if contrary NYISO evidence appears |
| NYISO Energy Marketplace training | NYISO | 2026 training material | Day-ahead/real-time market concepts; load forecasting; 5:00 a.m. Day-Ahead Market close; daily load forecast posting context | Training material; not an archive data dictionary | Use for market timing/context and as corroborating evidence for morning forecast posting |
...
| NYISO Load Forecasting Manual M-06, Version 5.1 | NYISO | Effective 10/27/2025 | ICAP/capacity load-forecasting methodology, weather normalization, data submission, revision history | Does not define short-term P-7 publication timestamps | Retain as supporting methodology documentation only |
...
- P-7 is the public ISO Load Forecast report; dated `isolf` artifacts contain rolling multi-day zonal forecasts.
- The P-7 public interface exposes a timezone-specific `Last Updated` timestamp for each dated forecast artifact.
- The project treats P-7 `Last Updated` as the best available public-source evidence for forecast availability.
- This interpretation is an explicit inference rather than a direct NYISO confirmation of the field's formal semantics.
- The ZIP-entry last-modified timestamp is retained as secondary provenance/audit information where available.
- The project uses the latest P-7 vintage with `Last Updated` strictly before 5:00 a.m. EPT D−1 that still contains the target hour.
...
| `data/interim/nyiso_hudson_valley_load_forecast_vintages.csv` | One row per vintage/target-hour pair | Audit and latest-eligible selection | 4,464 rows; must be regenerated when P-7 Last Updated metadata replaces ZIP-only timing |
...
| `data/processed/nyiso_hudson_valley_january_2025_modeling_ready.csv` | One row per target hour | January pre-modeling checkpoint | 744 rows; must be revalidated after P-7 timing update |
